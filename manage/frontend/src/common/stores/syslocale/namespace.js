...
import {list, select, create, update, deleteOne, get, getWithValue} from '../../services/syslocale/namespace'
...
class NameSpaceStore {
  ...
  @action
  fetchWithValue(payload, token) {
    const promise = getWithValue(payload, token)
    promise.then(response => {
      if (response.result === true && response.data) {
        this.current = response.data
      }
